...
trainDFpcs <- prep(trainDF[,-1],scale = c("none"), center = FALSE)
trainDFpcs <- pca(trainDF, method = "nipals", center = FALSE, nPcs = 5)

testDF <- 1 * testDF
testDFpcs <- prep(testDF[,-1],scale = c("none"), center = FALSE)
testDFpcs <- predict(object = trainDFpcs, newdata = testDFpcs, pcs = 2, pre = F)
